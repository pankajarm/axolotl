#!/usr/bin/env python3
"""
Training Monitor for Devstral-Small-2-24B-Instruct-2512-sft-v1
Monitors training progress, GPU stats, and estimates completion time.

Usage:
    python monitor_devstral2.py [--log-file LOG_FILE] [--interval SECONDS]
"""
...
# Using Devstral-Small-2507 (BF16) because Devstral-Small-2-24B (FP8) requires H100/4090
LOG_FILE = "/home/ubuntu/us-east-1-nano-chat-exp/logs/devstral2_full_20251215_203247.log"
OUTPUT_DIR = "/home/ubuntu/us-east-1-nano-chat-exp/outputs/Devstral-Small-2507-sft-v1/qlora-out"
...
def parse_training_log(log_file):
    ...
    # Find training progress lines
    # Pattern: {'loss': 0.xxx, 'grad_norm': x.xx, 'learning_rate': x.xx, 'epoch': x.xx}
    # Or step-based progress
    ...
    # Look for patterns like: Step X/Y, loss: X.XXX, epoch: X.XX
    lines = content.split('\n')
    
    for line in reversed(lines[-500:]):  # Check last 500 lines
        # Pattern for step progress with loss
        ...
        if pct_match and progress["current_step"] == 0:
            # Try to extract step from progress bar
...
def check_screen_running():
    ...
    try:
        ...
        return "devstral2_train" in result.stdout
    except Exception:
        return False
...
def print_status(log_file, clear=True):
    ...
    print(f"  🚀 Devstral-Small-2507 Training Monitor")
...
def main():
    parser = argparse.ArgumentParser(description="Monitor Devstral-2 training progress")
    parser.add_argument("--log-file", "-l", default=LOG_FILE, help="Path to training log file")
    ...
    if args.once:
        print_status(args.log_file, clear=False)
        return
    
    print("Starting training monitor... (Press Ctrl+C to exit)")
    try:
        while True:
            print_status(args.log_file)
