#!/usr/bin/env python3
"""
Smart Training Script for Devstral-Small-2-24B
...
Usage:
    python smart_train_devstral2.py [--quicktest-only] [--skip-quicktest]
"""
...
# Configuration
SCRIPT_DIR = Path("/home/ubuntu/us-east-1-nano-chat-exp")
AXOLOTL_DIR = SCRIPT_DIR / "axolotl"
VENV_PATH = SCRIPT_DIR / ".venv-ministral3"
LOG_DIR = SCRIPT_DIR / "logs"

# Using Devstral-Small-2507 (BF16) because Devstral-Small-2-24B (FP8) 
# requires compute capability >= 8.9 (H100/4090), but A100 is only 8.0
QUICKTEST_CONFIG = AXOLOTL_DIR / "examples/devstral2/devstral-small-2507-qlora-quicktest.yaml"
FULL_CONFIG = AXOLOTL_DIR / "examples/devstral2/devstral-small-2507-qlora.yaml"
...
def analyze_quicktest_results(log_content: str, output_dir: Path) -> dict:
    ...
    if "FineGrainedFP8Config" in log_content and "BitsAndBytesConfig" in log_content:
        results["success"] = False
        results["errors"].append("Cannot apply QLoRA to FP8 model - use LoRA instead")
        ...
        if gpu_stats:
            ...
            if mem_pct < 80:
                results["recommendations"].append(
                    f"GPU memory only {mem_pct:.1f}% utilized. Consider increasing micro_batch_size for better throughput."
                )
            elif mem_pct > 95:
                results["recommendations"].append(
...
def run_quicktest() -> tuple[bool, dict]:
    ...
    log_file = LOG_DIR / f"devstral2_quicktest_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def run_full_training():
    ...
    log_file = LOG_DIR / f"devstral2_full_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def main():
    parser = argparse.ArgumentParser(description="Smart training for Devstral-Small-2-24B")
    ...
    print_header("Smart Training: Devstral-Small-2507")
    print(f"{Colors.CYAN}Model:{Colors.ENDC} mistralai/Devstral-Small-2507")
    print(f"{Colors.CYAN}Adapter:{Colors.ENDC} QLoRA (4-bit quantization)")
    ...
    if not args.skip_quicktest:
        ...
        # Save results
        results_file = LOG_DIR / "devstral2_quicktest_results.json"
